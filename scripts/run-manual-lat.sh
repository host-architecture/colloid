...
for x in 0 0.5 1; do
    ...
    PYTHONPATH=$PYTHONPATH:$mio_path python3 -m mio $config-iso-x$x --stats_colloidlat &
    pid_stats=$!;
    sleep $duration;
    # killall record_stats;
    killall python3;
...
for x in 0 1; do
    ...
    pid_mio=$!;
    sleep 10;
    $gups_path/$gups_workload $gups_cores manual $x distribute > $stats_path/$config-bg-x$x.gups.txt 2>&1 &
    pid_gups=$!;
