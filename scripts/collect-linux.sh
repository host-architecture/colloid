...
config="colloidmtv2-linux-gupsr"
...
cat $stats_path/$config-iso.gups.txt | tail -n 30 | awk '{sum+=$1} END {print (sum/NR)*4096/1e9;}'

echo "App Throughput, with background traffic"
cat $stats_path/$config-bg.gups.txt | tail -n 30 | awk '{sum+=$1} END {print (sum/NR)*4096/1e9;}'
...
paste <(cat $stats_path/$config-bg.gups.txt | tail -n 30) <(cat $stats_path/$config-bg.stats.txt | tail -n 30) | awk '{print ($1*4096 - $6*64 - $7*64 + $10*4096 + $11*4096)/1e9, ($4*64+$5*64-$1*4096 + $6*64 + $7*64 - 2*$10*4096 - 2*$11*4096)/1e9, ($10*4096 + $11*4096)/1e9}' | awk '{sum1 += $1; sum2 += $2; sum3 += $3;} END {print sum1/NR, sum2/NR, sum3/NR}';
...
paste <(cat $stats_path/$config-iso.gups.txt | tail -n 30) <(cat $stats_path/$config-iso.stats.txt | tail -n 30) | awk '{print ($1*4096 - $6*64 -$7*64 + $10*4096 + $11*4096)/($1*4096)}' | awk '{sum1 += $1;} END {print sum1/NR}';
echo "App hit rate, with background traffic"
paste <(cat $stats_path/$config-bg.gups.txt | tail -n 30) <(cat $stats_path/$config-bg.stats.txt | tail -n 30) | awk '{print ($1*4096 - $6*64 -$7*64 + $10*4096 + $11*4096)/($1*4096)}' | awk '{sum1 += $1;} END {print sum1/NR}';
