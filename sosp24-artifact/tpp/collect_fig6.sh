...
printf "%-15s %-15s %-15s %-15s\n" "Latency" "tpp" "tpp+colloid" "Perf. Improvement"
...
for idx in 1 2 3; do
    u="${unc_regs[$idx]}"
    tpp_output=$(python3 $scripts_path/collect_ts.py tpp-unc$u-gups64-rw-app$i-bg$b gups | tail -n 30 | awk '{s += $1;} END {print (s/NR)*64*2/1e9;}')
    tpp_colloid_output=$(python3 $scripts_path/collect_ts.py tpp-colloid-unc$u-gups64-rw-app$i-bg$b gups | tail -n 30 | awk '{s += $1;} END {print (s/NR)*64*2/1e9;}')
    impr=$(awk -v num="$tpp_colloid_output" -v den="$tpp_output" 'BEGIN {print num/den;}')
