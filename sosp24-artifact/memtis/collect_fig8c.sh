...
for b in 0 5 10 15; do
    memtis_output=$(cat $results_path/memtis-cachelib-hememkv-app15-bg$b/output.log | grep -i "get       :" | awk '{print $3}' | tr -d '/,s')
    memtis_colloid_output=$(cat $results_path/memtis-colloid-cachelib-hememkv-app15-bg$b/output.log | grep -i "get       :" | awk '{print $3}' | tr -d '/,s')
