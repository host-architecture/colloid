...
#     paste <(echo "$(($b/5))x" ) <(python3 $scripts_path/collect_ts.py tpp-thp-gups64-rw-app$i-bg$b gups | tail -n 30 | awk '{s += $1;} END {print (s/NR)*64*2/1e9;}') <(python3 $scripts_path/collect_ts.py tpp-thp-colloid-gups64-rw-app$i-bg$b gups | tail -n 30 | awk '{s += $1;} END {print (s/NR)*64*2/1e9;}') |column -t;
# done;

printf "%-10s %-10s %-10s\n" "Intensity" "TPPw/THP" "TPPw/THP+colloid"
