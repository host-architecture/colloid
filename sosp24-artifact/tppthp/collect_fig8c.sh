...
for b in 0 5 10 15; do
    tpp_output=$(cat $stats_path/tpp-thp-cachelib-hememkv-app15-bg$b.app.txt | grep -i "get       :" | awk '{print $3}' | tr -d '/,s')
    tpp_colloid_output=$(cat $stats_path/tpp-thp-colloid-cachelib-hememkv-app15-bg$b.app.txt | grep -i "get       :" | awk '{print $3}' | tr -d '/,s')
